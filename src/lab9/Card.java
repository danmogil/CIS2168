...
/**
 * Implementation of a playing card. This class implements the Comparable
 * interface and compares cards by rank, with an undefined order for cards of
 * the same suit.
 */
public class Card implements Comparable<Card> {
	...
	private void check(String card) {
		if (card.length() != 2) throw new RuntimeException("WRONG CARD");
		if (!"23456789TJQKA".contains(card.substring(0, 1))) throw new RuntimeException("WRONG RANK");
		if (!"SHDC".contains(card.substring(1, 2))) throw new RuntimeException("WRONG SUIT");
	}
}